...
def col_name(idx):
    """列番号(1始まり)を列名に変換する。26→Z, 27→AA, 34→AH"""
    s = ""
    while idx > 0:
...
def build_pages(ws):
    """各ページの (番号/型式列/明細開始行/合計行/日付セル/使用済みか) を集める"""
    pages = []
    for prow, r0, r1, rsum, drow in BLOCKS:
        for cl, ci in COLS.items():
            pnum = ws.Cells(prow, ci + 1).Value
            if pnum is None:
                continue
            used = any(ws.Cells(r, ci).Value not in (None, "") for r in range(r0, r1 + 1))
            date_cell = f"{col_name(ci + 5)}{drow}"      # 日付は型式列+5（G/O/W/AE/AM）
