...
### 発注業務（注文書作成・DXF変換）をスキル化（2026-08-06）
- `.claude/skills/purchase-order`（見積書・図面→注文書Excel）と `.claude/skills/drawing-dxf`（SLDDRW→DXF一括）を新設。使い方は `.claude/skills/README.md`
- **見積書PDFはスキャン画像が多い**→ PyMuPDFで200dpiのPNG化して目視読取。読取後は**ページ小計・表紙合計と3段階検算**（今回125件・3,276,400円が全て一致）
- **図面PDFはCAD出力でテキストあり**→ 表題欄は縦書きで「ラベル列のすぐ左が値」。`値の中心がラベルのy範囲に入る`条件で列を厳密分離できる。PART NAME欄は他行が混ざるため**末尾トークンが品名**
- **対称品の判定**：QTY欄が「上記」＋図中に「数量：本図面品/図面対称品を各N個⇒計M個」→ 品名に「本図面品/図面対称 各N個」を付け、数量は計M個
- **品名の加工処理表記は削除**（(無電解ニッケル)(アルマイト)(チッカ)(焼入れ…)※処理無し）。ただし**(裏)などの部位表記は残す**
- **見積書の型式は誤記があり得る**。図面フォルダの実在図番と突合して発見した実例：
  - 見積`3089-400-555A`→図面`3089-400-55A` / 見積`3089-330-28A`(上下シリンダプレート)→図面`3089-330-38A` / 見積`3089-100-01A`(持ち上げブロック)→図面`3089-320-14A`
  - **同じ図番が別部品で2回出たら誤記を疑う**
- **SolidWorksは自動起動させない**（初期化中にCOM要求を受けると落ちる）。ユーザーに手動起動してもらい `GetActiveObject` で接続する。125件で515秒・全件成功
...
- **⚠️「ファイルが存在する＝成功」と判定してはいけない**：出力先PDFをAcrobatが開いていると削除も上書きもできず、**古い不良ファイルが残ったまま成功と誤判定**した（21件中1件）。**更新時刻が今回の開始時刻以降か＋先頭5バイトが `%PDF-` か**まで見る（`produced_ok()`）。作業前に「出力先のPDFを閉じて」と伝える
  - なお**Acrobatを閉じた瞬間に削除保留が実行されてファイルごと消える**（削除→上書き失敗の順で起きるため）。閉じてもらってから該当分だけ `--only <図番>` で作り直す

### DXFとPDFの照合（`match_dxf_pdf.py`・2026-08-12）

- 支給前チェックは「**図番が ファイル名／DXF内／PDF内 の3つで一致**＋**PDFの寸法値がすべてDXFにある**」で判定する。21件を1分で照合できた
- **⚠️ezdxfで寸法値を拾うにはDIMENSIONの `geometry` ブロックをたどる必要がある**。モデル空間とINSERT先だけ見ると寸法文字が丸ごと抜け、**一致率52〜81%という誤検出**になった（実際は100%一致）。`entity.dxf.geometry` の `*D...` ブロックの中にMTEXTがある
- **DXF ⊇ PDF で判定する**（完全一致にしない）。DXFには非表示レイヤの公差表（上限/下限, 3,6,30,120…）など印刷されない要素も入るため
- **ラスタ化PDFは文字が取れない**ので目視。表題欄（右下）だけ切り出せば図番・品名・材質・数量が確認できる。**これらのPDFはページ回転90°が入っており、`show_pdf_page(clip=)` では切り出し位置がずれる → `page.get_pixmap(clip=..., dpi=130)` を使う**
- 複数件の目視は**1枚のPNGに縦積みして一度に確認**すると速い（`Pixmap.set_origin()`＋`copy()`）
...
### farewell-reception 返金ロジック＝段階水位方式（2026-07-24）
- **逆転現象**：役職者に余剰金を一律按分すると、課長の実質負担が一般社員より安くなる逆転が起きる（NG）
- **解決＝段階水位方式**：①一般に設定額返金 ②floor（一般社員の実質負担）を下限に、**ご支援金の少ない役職グループから順に**floorまで返金し、余りを支援金の多いグループへ ③各人の返金=max(0, 個人負担due−グループ水位)。調整額を多く払った人は低支援金グループにいれば優先的に多く戻る
- アプリ実装：`calc.ts` の `execTierBurden`（support額→実質負担のMap）＋`rankRefund(due,grp,support,s,useActual)`。グループ内は二分探索water-level、グループ間はプール引継ぎ
- **Excel実装（同ロジック）**：ヘルパーセル（列P〜R・印刷範囲外）で floor/pool/低ティアcap/水位/残プール/高ティア水位を段階計算。低ティアcap=`人数×(規定負担−floor)+SUMIF(調整額)`（負担額合計H列は空行""でSUMPRODUCTが#VALUE!になるので、人数×規定＋調整SUMIFで回避）。設定シート実質負担=`MIN(規定負担, 水位)`、0名役職は`IF(COUNTIF=0,"返金なし",…)`
  - **⚠️Excelは2ティア（課長次長／部長以上）ハードコード**。役職グループ構成を変えたら式の見直しが必要。汎用計算はアプリが正
  - **アプリとExcelの金額差はデータ差**（参加者数・費用）で生じる。ロジックは同一。完全一致には両者の人数・費用を揃える
- **0名の役職は「返金なし（対象者なし）」表示**（アプリ・Excel両方）
