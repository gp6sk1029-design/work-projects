...
| タブ | 機能 |
|---|---|
| ✅受付 | 来場チェック・集金チェック・✏️実徴収額変更・未収絞り込み・検索・集計バー（従来機能） |
| 👥参加者 | 追加・編集・削除。役職を選ぶと会費・ご支援金が自動設定。招待/欠席の区分あり。**調整額（イレギュラー支払い）**入力可 |
| 💰費用 | 固定費・変動費の管理。予算は「1人当たり×出席者数」か「総額」の2方式。実績入力で差額管理 |
| 📊収支 | 収支サマリー（予算/実績2本立て）・余剰金・返金配分（一般=一律・役職者=按分）・役職別実質負担 |
| ⚙️設定 | イベント情報・役職別会費テーブル（**モーダルで会費/ご支援金/グループ編集**）・返金設定・**イベント切替/新規作成**・**保存/読込（JSON）**・**データリセット** |

## データの保存・読み込み・リセット（設定タブ）

- **💾 保存**：現在のイベント一式（イベント情報・役職・費用・参加者・受付結果）を1つのJSONファイルにダウンロード（`/api/export`）。別PCへの移動・バックアップ・過去イベントの振り返りに
- **📂 読み込み**：JSONファイルをアップロードすると**新しいイベントとして復元**（`/api/import`）。既存データは消えない
- **リセット**（`/api/reset`）：①受付状況だけ（来場・集金クリア、参加者は残す）②参加者を全削除 ③費用を全削除。次のイベント準備・使い回し用

## 受付タブのチェックUI

来場・集金を**2つの隣接トグル**（チェックボックス＋ラベル「来場」「集金」）＋列見出しで明示。誰でも「この2つをタップしてチェック」と分かる設計。

## イレギュラー支払い（調整額）

規定額（会費＋ご支援金）と異なる金額を払う人に対応。参加者に `adjust`（調整額）を持たせ、
**徴収額 = 会費 ＋ ご支援金 ＋ 調整額**（＋多め／−割引）で自動計算する。
- 受付タブの✏️ …「実際に受け取る金額」を入力 → 差額を自動で調整額に記録
- 参加者タブ … 調整額を直接入力（事前登録）。理由は備考へ
- 収支タブ … イレギュラー支払いの人数・純増減を注記表示（収入合計に反映済み）
- 例：中村次長は ＋10,000円（計20,000円）で登録済み
