...
### 4. DXFとPDFの照合（支給前の最終チェック）
```bash
python scripts/match_dxf_pdf.py --dir "<DXFとPDFが入ったフォルダ>"
```
- DXFとPDFが**1対1で揃っているか**（片方だけ無いものを検出）
- **図番が ファイル名／DXF内／PDF内 の3つで一致**するか
- 品名がDXF・PDFの両方にあるか
- **PDFに書かれた寸法値がすべてDXF側にもあるか**（DXFには非表示レイヤも入るので「PDF⊆DXF」で判定）
- ラスタ化されたPDFは文字が取れないので「要目視」として一覧に出す
  → `--png-out <フォルダ>` を付けると画像化される。**表題欄（右下）だけ切り出して図番を確認**すれば足りる

> 💡 ラスタPDFの表題欄を切り出すときは `page.get_pixmap(clip=..., dpi=130)` を使う。
> これらのPDFは **ページ回転90°** が入っているため、`show_pdf_page(clip=...)` では
> 切り出し位置がずれる。
